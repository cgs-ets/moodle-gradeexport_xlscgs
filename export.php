...
require_once 'grade_export_xlscgs.php';

$id                = required_param('id', PARAM_INT); // course id
$PAGE->set_url('/grade/export/xlscgs/export.php', ['id' => $id]);
...
if (!empty($CFG->gradepublishing) && !empty($key)) {
    $actionbar = new \core_grades\output\export_publish_action_bar($context, 'xlscgs');
    print_grade_page_head($COURSE->id, 'export', 'xlscgs',
...
$mform = new grade_export_form(null, ['publishing' => true, 'simpleui' => true, 'multipledisplaytypes' => true]);
$formdata = $mform->get_data();
$export = new grade_export_xlscgs($course, $groupid, $formdata);
...
if (!empty($CFG->gradepublishing) && !empty($key)) {
    ...
    $event = \gradeexport_xls\event\grade_exported::create(['context' => $context]);
    $event->trigger();
    $export->print_grades();
}
